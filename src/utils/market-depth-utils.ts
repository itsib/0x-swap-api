...
export function getBucketPrices(
...
export function getSampleAmountsFromDepthSide(depthSide: MarketDepthSide): BigNumber[] {
...
export function sampleNativeOrders(
...
export function normalizeMarketDepthToSampleOutput(depthSide: MarketDepthSide, side: MarketOperation): MarketDepthSide {
    // Native is not a "sampled" output, here we convert it to be a accumulated sample output
    ...
    const sampleAmounts = getSampleAmountsFromDepthSide(depthSide);
    const nativeSamples = sampleAmounts.map((a) => {
        const sample = sampleNativeOrders(
...
export function calculateStartEndBucketPrice(
  depthSide: MarketDepthSide,
  side: MarketOperation,
  endSlippagePercent = 20,
): [BigNumber, BigNumber] {
    ...
    if (side === MarketOperation.Sell) {
        ...
        worstBestInBracket = bestInBracket.times((ONE_HUNDRED_PERC - endSlippagePercent) / ONE_HUNDRED_PERC);
    } else {
...
export function distributeSamplesToBuckets (depthSide: MarketDepthSide, buckets: BigNumber[], side: MarketOperation) {
    ...
    return allocatedBuckets.map((b, bucketIndex) => {
...
export function calculateDepthForSide(
  ...
  maxEndSlippagePercentage: number = MARKET_DEPTH_END_PRICE_SLIPPAGE_PERC,
    ...
    const depthSide = normalizeMarketDepthToSampleOutput(rawDepthSide, side);
    const [startPrice, endPrice] = calculateStartEndBucketPrice(
      depthSide,
      side,
      maxEndSlippagePercentage,
    );
    const buckets = getBucketPrices(startPrice, endPrice, numBuckets, bucketDistribution);
    return distributeSamplesToBuckets(depthSide, buckets, side);
}
