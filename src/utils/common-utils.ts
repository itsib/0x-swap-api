 ...
 */
export function isValidJsonRpcResponseOrThrow(response: JsonRpcResponse, payload: any = undefined): boolean {
...
export function chunkByByteLength<T>(items: T[], maxByteLength: number): T[][] {
...
export async function runWithTimeout<T>(fn: () => Promise<T>, timeoutMs: number): Promise<any> {
...
export function setAsyncExcludingImmediateInterval(
    fn: () => Promise<void>,
  intervalMs: number,
...
export function calculateCallDataGas(bytes: string) {
